...
\definecolor{gray-table}        {HTML}{000000}  % 666666  for gray, 000000 for black
\definecolor{gray-background}        {HTML}{EAEAEA}
...
\title{Checklist to Transparently Define Test Oracles for TP, FP, and FN Objects in Automated Driving}
...
The present work was conducted independently without related \href{https://ko-fi.com/michaelhoss}{funding}.
}
...
% TODO incorporate Yannick's feedback:
...
% TODO I could frame this as functional and non-functional aspects of a test oracle
...
Ideally, novel perception metrics combine all desired properties such that they are helpful to both perception system developers \textit{and} verification and validation engineers. 
...
\subsubsection{Object under Test (OuT)} \label{def:out} From ISO/TR 4804:2020 \cite{ISO_TR_4804_2020}. Here: perception subsystem of a driving automation system. It comprises both hardware and software and it outputs an object list in each time step. 
...
Similarly, \cite{Mori2023relevance} and \cite{Storms2023relevance} determine directly which objects are relevant for perception.
...
For example, the excluded pedestrian D in Fig.~\ref{fig:top_down_all} might still be running towards the intersection and thereby influence other trajectories.
...
While the object list matching in Fig.~\ref{fig:top_down_all} is straightforward, there may also be complex scenes like a dense crowd of pedestrians, where each bounding box has many potential matches. % and the matrix of matching object distances has non-sparse blocks. 
...
%In doing so, outdated time stamps get traded in for worsened existence and accuracy properties.
...
% Issue: what is the correct wording for existence probability/confidence? Like, an object exists with 0.9, but these 0.9 were computed with a "2nd order" confidence of only e.g. 0.3. See also dempster shafer theory and Madala 2023).
% For now, I will call these things just all confidences and carry on.
...
However, even the level of detail of Table~\ref{table:case_study} is still not sufficient for a transparent oracle definition and leaves open uncertainties. %, especially in terms of ReS data processing and labeling (Sec.~\ref{sec:basic_ref_labeling}).
...
% CONTEXT
\newcommand{\scenariosDrone}{\textcolor{gray-table}{Ego vehicle statically observes an urban intersection from a sidewalk.}}
\newcommand{\outDrone}{\textcolor{gray-table}{Single lidar sensor with integrated object detection and tracking}}
\newcommand{\furtherDrone}{\textcolor{gray-table}{Modeling of how OuT deviates from ReS regarding TP object states}}

\newcommand{\scenariosChallenge}{\textcolor{gray-table}{Ego vehicle dynamically participates in various urban traffic scenarios.}}
\newcommand{\furtherChallenge}{\textcolor{gray-table}{Computation of metrics such as AMOTA and AMOTP \cite{weng2019baseline} for the online leaderboard}}
\newcommand{\outChallenge}{\textcolor{gray-table}{Submitted detection and tracking algorithms that operate on either camera-only, lidar-only, or all raw sensor data}}

\newcommand{\scenariosCol}{\textcolor{gray-table}{Scenarios}}
\newcommand{\outCol}{\textcolor{gray-table}{OuT}}
\newcommand{\furtherCol}{\textcolor{gray-table}{Further evaluation}}
...
\newcommand{\basicDroneReSLabling}{Offline post-processing as in the inD dataset: correcting lens distortion, stabilizing wind disturbances, semantic segmentation by UNet, vehicle localization, then tracking and smoothing by Kalman Filter. Human annotators are mostly involved in the training of the ReS, less in the release of ReS labels.}
...
\newcommand{\basicDroneMultiObjMatching}{Entire tracks are matched (see below). \textit{Not published:} Multiple OuT tracks can be matched to one ReS track (also at the same time), but only one ReS track can be matched to an OuT track (1:n).}
...
\newcommand{\tempDroneIncomplete}{\textit{Not published:} Two or more incomplete OuT tracks can be matched with one ReS track. FNs were not explicitly computed.}
...
\newcommand{\basicChallengeFoV}{ReS FoV $\supseteq$ OuT FoV, both from ego perspective\tnote{b}. Ranges are partially defined, but the oracle cuts off ReS and OuT objects beyond class-specific distances.}
...
\newcommand{\basicChallengeReSHW}{ReS HW\tnote{b} (cameras, lidar, radar, GNSS+IMU) $\supseteq$ OuT HW}
\newcommand{\basicChallengeReSLabling}{See human annotator instructions\tnote{c}. They are clear and detailed, but cannot rule out all uncertainties (when ``reasonably sure" about object shape and location, use ``best judgment" on bounding box properties). ReS objects must have at least 1 lidar or radar point. ``Multiple validation steps" \cite{caesar2019nuscenes} that are not further specified. %, which might lead to confusion in specific cases\tnote{c}.
}
...
\newcommand{\basicChallengeGeometrAlign}{Calibration of lidar, camera, and radar w.r.t. ego coordinate system; without accuracy estimate $\rightarrow$ ReS-OuT-alignment might be non-perfect for the camera-only OuT.}
...
\newcommand{\basicChallengeMultiObjMatching}{Metrics from \cite{Bernardin2006mot_metrics} use 1:1 matching and Munkres' algorithm (see \cite{bernardin2008evaluating}).}

\newcommand{\tempChallengeSync}{Online synchronization of lidar (\SI{20}{\hertz}) and camera (\SI{12}{\hertz}), but evaluation only on keyframes (\SI{2}{\hertz}) where the sensors coincide. Radar operates on \SI{13}{\hertz}\tnote{d}. ReS tracks and OuT tracks are linearly interpolated ``to avoid excessive track fragmentation from lidar/radar point filtering"\tnote{a}.}
\newcommand{\tempChallengeMatching}{Metrics from \cite{Bernardin2006mot_metrics} keep a match over subsequent time steps for as long as it is below the threshold, even if another, new match candidate would have less cost.}
...
\begin{table*}[htbp]
	...
	\begin{tabularx}{\linewidth}{
		...
		\toprule
		\multicolumn{2}{>{\hsize=\dimexpr0.4\hsize+0.4\tabcolsep+\arrayrulewidth\relax}X}{\textbf{Criterion of checklist}}                                         & \textbf{Evaluation of nuScenes tracking challenge\tnote{a}} & \textbf{Obtaining TPs from a drone \cite{Krajewski2020UsingDrones}} \\ \midrule
 \parbox[t]{2mm}{\multirow{3}{*}[-0.4cm]{\rotatebox[origin=c]{90}{(\textit{Context})}}} & \scenariosCol                                           & \scenariosChallenge                  & \scenariosDrone                                                                 \\ \cline{2-4}
	...
	\begin{tablenotes}
		...
		%	\item [x] \href{https://github.com/nutonomy/nuscenes-devkit/issues/366}{https://github.com/nutonomy/nuscenes-devkit/issues/366}
	\end{tablenotes}
\end{threeparttable}
\end{table*}
...
% TODO: maybe include what else I found out (besides the research questions)
...
Especially, choosing which objects or areas are relevant (Sec. \ref{sec:basic_areas}), as well as reliably labeling ReS data (or ReS data of ReS data; Sec. \ref{sec:basic_ref_labeling}), seems necessary for ADS perception testing even if no explicit TPs, FPs, or FNs are computed. %seem to be universal challenges or ADS perception testing.
