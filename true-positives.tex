...
Since clear definitions are needed for their comparability and for a potential usage within a safety case, this paper provides a set of relevant criteria for identifying TP, FP, and FN objects. % in an ideally unambiguous way.
...
We find it challenging to formally characterize all aspects relevant for identifying TP, FP, and FN due to the symbol grounding problem in an open context and due to degrees of freedom in the oracle implementation.
...
We currently assume that an ideal test oracle reproduces such human expert statements, as this would maximize the human comprehensibility of safety assurance.
% However, to compare test results, the oracle should be specifiable as much as possible, which means it should contain as little arbitrary human intuition as possible. 
Consequently, our method is to encode our human intuition for a test oracle into criteria for its unambiguous technical definition. 
Furthermore, we 
oracle aspects from the current literature
implementation details from our own experience.

TODO other criteria for exclusion like TTC too high.
...
We refine the criteria for defining test oracles as we describe two real-world test oracles through them (Sec. \ref{sec:case_studies}). 
...
TODO briefly cite the recent zone approaches from my rolling review.
...
\begin{itemize}
	\item Volk et al. \cite{Volk2020metric}
	\item Lyssenko et al. \cite{Lyssenko2021relevance}
	\item Wolf et al. \cite{Wolf2021people}
	\item Bansal et al. \cite{Bansal2021riskrankedrecall}
	\item Topan et al. \cite{Topan2022zones}
\end{itemize}
and the following metrics are oracle-independent (instead, they typically depend on a behavior policy and a threat metric):
\begin{itemize}
	\item Philion et al. \cite{Philion2020planner_centric}
\end{itemize}
...
For completeness, this section contains those relevant literature sources that have not been part of earlier sections.

Florbäck et al. \cite{Florbaeck2016.matching.offline} and Sondell and Svensson \cite{Sondell2018} investigate how machine learning can reproduce human associations of OuT objects to ReS objects.
