...
To date, there exists a common notion of TPs, FPs, and FNs for this application, but apparently no published way to comprehensively characterize entire real-world test oracles. 
...
Besides labeling policies of the test set, we cover sensor hardware, fields of view, safety-relevant areas, matching criteria, temporal and probabilistic issues, and other aspects.
While our checklist for oracle definition can hardly be formalized, it can help practitioners describe their oracles with maximized clarity, which makes statements on object perception more reliable and comparable.
...
We are aware of literature on test oracles that generally treats software testing \cite{Barr2015oracle} and that discusses safety-oriented ADS perception testing \cite{Abrecht2021testing}.
However, we have not yet found a comprehensive compilation of aspects that constitute an implemented test oracle for our application.

Related publications that focus on parts of the entire test oracle (e.g. only labeling policies, safety-relevant zones, or object matching) are referred to directly in the corresponding subsections of the oracle definition (Sec.~\ref{sec:criteria}) or of the discussion (Sec.~\ref{sec:discussion}).
