...
On the one hand, the nuScenes 3D tracking challenge\footnote{\url{https://www.nuscenes.org/tracking}}
...
\newcommand{\basicDroneOcclusion}{\textcolor{red}{Not published (how did Fabian do it?)}}
...
\newcommand{\basicDroneObjDistance}{2D center distance on the ground plane}
\newcommand{\basicDroneMultiObjMatching}{Only car-car matching of entire tracks (see below)\textcolor{red}{. What if two OuT tracks were below that threshold for a ReS track?}}

\newcommand{\tempDroneSync}{Asynchronous OuT and ReS at \SI{25}{\hertz}; synchronization in post-processing through a mutually captured visual signal; resulting accuracy loss $\approx$\,\SI{0.14}{\meter}. \textcolor{red}{What did we do if the signal was between two frames for one system, but on one frame for another? Interpolation?}}
\newcommand{\tempDroneMatching}{Entire tracks are matched if their mean centroid distance (see above) during their lifetimes $<$\,\SI{1.5}{\meter}.}
\newcommand{\tempDroneIncomplete}{\textcolor{red}{Not published (related to occlusion handling, how did Fabian treat missing OuT frames in the middle of a ReS object?)}}
\newcommand{\tempDroneDelays}{Any temporally overhanging track parts are discarded by counting them as FNs or FPs.}
...
\newcommand{\probDroneExistClass}{\textcolor{red}{OuT objects were considered if their classification with highest probability was ``car" at least once during their lifetime (like this, or different?). All existence probabilities were considered without any threshold.}}
...
\newcommand{\basicChallengeReSLabling}{See human annotator instructions\tnote{a}. They are clear and detailed, but cannot rule out all uncertainties (when ``reasonably sure" about object shape and location, use ``best judgment" on bounding box properties). ReS objects must have at least 1 lidar or radar point. ``Multiple validation steps" \cite{caesar2019nuscenes}. %, which might lead to confusion in specific cases\tnote{b}.
}
\newcommand{\basicChallengeAreas}{No evaluation beyond class-specific maximum detection and tracking ranges; no evaluation of bikes and motorcycles inside bike racks.}
...
\newcommand{\basicChallengeObjDistance}{2D center distance on the ground plane; threshold of \SI{2}{\meter}}
...
\newcommand{\tempChallengeSync}{Online synchronization of lidar (\SI{20}{\hertz}) and camera (\SI{12}{\hertz}), but evaluation only on keyframes (\SI{2}{\hertz}) where the sensors coincide}
...
\newcommand{\probChallengeExistClass}{Metrics in further evaluation \cite{weng2019baseline} average over 40 different recall values, which stem from a varied existence probability threshold of the oracle. \textcolor{red}{How are classes treated?} % Note: the existencd probability of a TP object is called "score" in https://github.com/xinshuoweng/AB3DMOT/blob/master/scripts/KITTI/evaluate.py#L365
}
...
\newcommand{\furtherEvaluation}{Context: further evaluation after TP, FP, FN }
\newcommand{\exampleOuTs}{Context: OuT}
...
\begin{table*}[t]
	...
	\begin{tabularx}{\linewidth}{
		...
		\parbox[t]{2mm}{\multirow{3}{*}[-0.2cm]{\rotatebox[origin=c]{90}{Probabilistic}}}    & \ref{sec:prob_fov} FoVs                             & \probChallengeFoV                    & \probDroneFoV                                                                   \\ \cline{2-4}
		                                                                             ...
		                                                                             & \ref{sec:prob_thresholding} Obj. exist. \& class.   & \probChallengeExistClass             & \probDroneExistClass                                                            \\ \midrule
		\multicolumn{2}{>{\hsize=\dimexpr0.4\hsize+0.4\tabcolsep+\arrayrulewidth\relax}X}{\furtherEvaluation}                              & \furtherChallenge                    & \furtherDrone                                                                   \\ \cline{1-4}
		\multicolumn{2}{>{\hsize=\dimexpr0.4\hsize+0.4\tabcolsep+\arrayrulewidth\relax}X}{\exampleOuTs}                              & \outChallenge                    & \outDrone                                                                   \\ \bottomrule
	\end{tabularx}
	\begin{tablenotes}
		\item [a] \href{https://github.com/nutonomy/nuscenes-devkit/blob/master/docs/instructions_nuscenes.md}{https://github.com/nutonomy/nuscenes-devkit/blob/master/docs/instructions\_nuscenes.md}
	%	\item [b] \href{https://github.com/nutonomy/nuscenes-devkit/issues/366}{https://github.com/nutonomy/nuscenes-devkit/issues/366}
	\end{tablenotes}
	\end{threeparttable}
\end{table*}
