...
\item A discussion of the relevance of TP, FP, and FN for the driving task and safety
\end{itemize}


THIS IS IT I GUESS. THE REST OF THE INTRODUCTION CAN BE REALLY SHORT. 
...
\subsubsection{Object under Test (OuT)} Perception subsystem of an ego vehicle / subject vehicle that contains a driving automation system. Note: The ``object" in OuT must not be confused with the more general \textit{object} defined above (Sec. \ref{def:object}).

\subsubsection{True Positive (TP)} The circumstance that an object in the data under test matches with an object in the reference data.

\subsubsection{False Positive (FP)} The circumstance that an object in the data under test does not match with any object in the reference data. 

\subsubsection{False Negative (FN)} The circumstance that no object in the data under test matches with a given object in the reference data.

\subsubsection{Matching method} Those parts of the test method that influence the identification of TPs, FPs, and FNs. % based on an object list under test and a reference object list. 
...
\subsubsection{Matching result} General term for the mutually exclusive circumstances TP, FP, FN.  
...
\subsubsection{Object association} As part of a perception algorithm, deciding which existing object track a novel object detection belongs to. Not to be confused with \textit{matching}, which is a concept of the test method (adopted from \cite{Luiten2020hota}).

\subsubsection{Oracle} The mechanism that determines whether a test passes or fails. See also the oracle survey by Barr et al. \cite{Barr2015oracle}.
