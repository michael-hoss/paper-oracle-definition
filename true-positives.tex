...
While one could make safety-relevant metrics less complex (top-down approach), the present paper aims at making straightforward metrics better usable for safety assurance (bottom-up approach). 
...
In particular, since the \textit{symbol grounding problem} prevents a full specification of road user classes \cite{Salay2019partialspecifications}, also the test oracle for detecting these road users cannot be fully specified.

Even a theoretical solution to this challenge of test oracles for machine learning would still leave open several degrees of freedom of its practical implementation (ReS hardware, FoVs, temporal synchronization, geometrical alignment).
...
Probably I also want to cite (and obtain in the first place) \cite{Frtunikj2019engineering}. 
...
First, only related work here that is about the actual definition of TPs, FPs, and FNs. 
...
Some other literature that treats test oracles: \cite{Abrecht2021testing}.
