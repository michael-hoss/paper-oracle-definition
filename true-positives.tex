...
\item {RQ4}: \textit{Do TP/FP/FN-independent metrics avoid the given oracle challenges?}
\end{itemize}
...
\subsection{RQ4: Do TP/FP/FN-independent metrics avoid the mentioned oracle challenges?}
...
do avoid the arbitrariness of some checklist aspects:
\begin{itemize}
\item stuff
\end{itemize}
but otherwise, face the same challenges.


Even when using oracle-independent metrics, the same challenges in deciding which objects or areas are safety-relevant or not come into play (either as safety-relevant areas for TP metrics or as some behavior policy in terms of which the tested object list is compared to the ReS object list).

Also: Task-oriented metrics such as PKL might be safety-relevant, but not necessarily sufficient for safety evidences.
