...
\title{Relevant Criteria for Defining TP, FP, and FN in Dependability Assessment of Object Perception}
...
Test methods for the perception subsystem of driving automation systems commonly determine true-positive (TP), false-positive (FP), and false-negative (FN) objects. % general term for TP, FP, FN
...
Therefore, this paper provides a set of relevant criteria for defining TP, FP, and FN objects. % in an ideally unambiguous way.
% The criteria generally cover 
%that cover the generation of reference objects and their association to objects under test. 
Emphasized details are the level of the objects (e.g. detections or tracks), reference sensors, labeling policies, object distance functions, % in state space including penalties and thresholds, 
...
Nevertheless, we encourage practitioners to define TP, FP, and FN objects as formally as feasible. 
Remaining uncertainties in their definition should be explicitly named and considered in the safety context.
...
Also much research in safety assurance. 
What is needed for a decomposition approach: claim that perception subsystem is reliable and allows safe vehicle behavior. Or at least claim that a sensor model has representative failure rates.
...
Therefore, it is highly unclear when a FP or a FN also denote a failure that is relevant to the driving task. 
...
\begin{itemize}
\item A set of criteria to be used for the definition of TP, FP, and FN
