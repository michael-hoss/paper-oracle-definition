...
\newcommand{\tempDroneSync}{Both OuT and ReS operate at \SI{25}{\hertz}. Synchronization during post-processing by means of a mutually captured visual signal. Accuracy loss $\approx$\,\SI{0.14}{\meter}.}
...
\newcommand{\tempDroneIncomplete}{not published (related to occlusion handling, how did Fabian treat missing OuT frames in the middle of a ReS object?)}
\newcommand{\tempDroneDelays}{Any temporally overhanging track parts are discarded as FNs or FPs.}

\newcommand{\probDroneFoV}{OuT FoV is acknowledged to have a fuzzy range, but its fuzziness is not explicitly modeled.}
\newcommand{\probDroneObjDistance}{-}
\newcommand{\probDroneExistClass}{OuT objects were considered if their classification with highest probability was ``car" at least once during their lifetime. All existence probabilities were considered without any threshold.}

\newcommand{\furtherDrone}{Modeling of how OuT deviates from ReS regarding TP object states}



\newcommand{\basicChallengeFoV}{OuT FoV $\subseteq$ ReS FoV. No clear FoV boundaries defined.}
\newcommand{\basicChallengeOcclusion}{Only objects with at least 1 lidar or radar point are labeled, which implictly defines the occlusion handling.}
\newcommand{\basicChallengeReSHW}{ReS HW (cameras, lidar, radar) is a superset of OuT HW}
\newcommand{\basicChallengeReSLabling}{See labeling instructions\tablefootnote{\href{https://github.com/nutonomy/nuscenes-devkit/blob/master/docs/instructions_nuscenes.md}{https://github.com/nutonomy/nuscenes-devkit/blob/master/docs/instructions\_nuscenes.md}}. No labeling of bikes and motorcycles in bike racks.}
...
\newcommand{\basicChallengeGeometrAlign}{same sensors, therefore perfectly aligned}
...
\newcommand{\furtherChallenge}{Computation of metrics such as AMOTA and AMOTP for the online leaderboard}
...
\begin{table*}[t]
	...
	\begin{tabularx}{\linewidth}{
		...
		\parbox[t]{2mm}{\multirow{8}{*}[-2cm]{\rotatebox[origin=c]{90}{Basic}}} & \ref{sec:fov_ref} FoVs                              & \basicChallengeFoV                   & \basicDroneFoV                                                                  \\ \cline{2-4}
		...
		\multicolumn{2}{>{\hsize=\dimexpr0.4\hsize+0.4\tabcolsep+\arrayrulewidth\relax}X}{\furtherEvaluation}                         & \furtherChallenge                    & \furtherDrone                                                                   \\ \bottomrule
	\end{tabularx}
\end{table*}
