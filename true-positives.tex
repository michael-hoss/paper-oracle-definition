...
Third, even if OuT and ReS share the same sensors, their sensor ranges might not have clear-cut limits, but instead degrade probabilistically with increasing distance. In this probabilistic zone of performance degradation, the OuT's detection algorithm might propose object hypotheses where the ReS already refuses to label objects.
...
If only the perception software is tested, then one might want to remove all objects from overlapping areas of the ReS to enable a fair evaluation.
...
To avoid needless test efforts, one could define no-test-areas, where no evaluation of TPs, FPs, and FNs takes place (Fig.~\ref{fig:top_down_all}, pedestrian D). TODO cite kitti.
...
While the previous section (Sec. \ref{sec:oracle_simple}) only treated object representations in one single time frame, this section highlights how the test oracle can deal with temporal aspects.  
...
The three examples of Fig.~\ref{fig:timeline} show pairs of tracks that fulfill the geometrical criteria for being matched to one another.
Now, temporal aspects of the test oracle decide how exactly these tracks get matched.

If tracks are split up for better-matching sub-sequences, then in the middle of Fig.~\ref{fig:timeline}, the late part of the first ReS track can be matched with the early part of the second OuT track. 
...
% already covered in FoV above.
% But I might want to bring that only here because that is clearer distinction between deterinistic and probabilistic.
