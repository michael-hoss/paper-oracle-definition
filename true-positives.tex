...
Third, even if OuT and ReS share the same sensors, the OuT's detection algorithm might propose object hypotheses on distant raw data where the ReS refuses to label objects.
...
Certain areas on or next to the road surface might be irrelevant to the traffic scenario and therefore not worth while perceiving. 
...
However, these uncertainties can skew the reference objects' positions, which affects the distance function values for object matching (Sec. \ref{sec:distance_function}), which, in turn, affects the resulting TPs, FPs, and FNs.  
...
Mention truck H in Fig.~\ref{fig:top_down_all}. 
...
Mention cyclists F in Fig.~\ref{fig:top_down_all}. 
...
This work was conducted privately without funding. 
The author would like to thank those social media users who interacted with his posts about the given topic for the feedback and inspiration.
