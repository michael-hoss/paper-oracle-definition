...
\newline
Despite being the only author, the first person plural (``we") is used in the paper to comply with a common style of scientific writing. 
}
...
The task of object-based environment perception for automated driving receives much focus through public perception challenges. % that benchmark different approaches.
...
Common metrics of such leaderboards consist of aggregations of true-positive (TP), false-positive (FP), and false-negative (FN) objects. 
...
It is dependable in case of a sufficiently low mean time between events that induce vehicle-level failures. 
...
In contrast to the most popular perception leaderboard metrics, safety-oriented metrics tend to be more complex and incomprehensible due to their additional consideration of the downstream driving function and safety in road traffic. 
...
This requires solving the tradeoff between straightforward safety-agnostic metrics and highly complex safety-relevant metrics. 
%comprehensible to humans, and straightforward to compute all at the same time. 
The present paper contributes to solving this tradeoff by providing guidance to define 
CONTINUE HERE
...
\begin{itemize}
\item A set of criteria to define how a test method (or a test oracle) identifies TP, FP, and FN (Sec. \ref{sec:criteria})
...
\begin{figure*}[t]
	...
	\caption{Test oracle for identifying TPs, FPs, and FNs, mapped into the taxonomy for ADS perception testing \cite{Hoss2022review, stellet2015testing}.
...
\begin{figure*}[t]
	...
	\caption{Aspects influencing the test oracle for TPs, FPs, and FNs. TODO: include 1) an object only in the fov of the OuT that the OuT did not perceive. 2) an object outside all fovs
	}
	\label{fig:top_down_all}
\end{figure*}

The aspects that define the test oracle for TPs, FPs, and FNs, are illustrated in Fig. \ref{fig:top_down_all}.

% USE NATURAL LANGUAGE HERE!

In this section, we intentionally use natural language and avoid a more specific or more technical description of these aspects in order to keep it generally applicable to as many test oracles as possible without prescribing an internal structure. 
